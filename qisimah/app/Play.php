...
//    	$plays = [];
    ...
    {
        $_regions = Region::where('country_id', $country_id)->get(); // Get all regions in Country
        $_total = 0;
        $_start = Carbon::parse($start);
        $_end = Carbon::parse($end);
...
//        return Play::select(DB::raw('count(*) as plays, stream_id'))->with('broadcasters')->where('file_id', $file_id)->groupBy('stream_id')->orderBy('plays', 'desc')->get();
		...
		Play::select(DB::raw('count(*) as plays, stream_id'))
			->with('broadcaster')->where('file_id', $file_id)
			->whereBetween('created_at', [$_start->startOfDay(), $_end->endOfDay()])
			->groupBy('stream_id')->orderBy('plays', 'desc')
			->chunk(500, function ($plays) use ($_regions, $_total, &$total_plays, &$regional_plays) {
				foreach ($plays as $play) {
					...
					$regional_plays = $_regions;
					$total_plays = $_total;
				}
			});

        return [$regional_plays, $total_plays];
    }
